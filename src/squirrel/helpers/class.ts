 ...
 * @param path
 */
export function getConstructorDefinition(path: NodePath<ClassBody>) {
    return path.get('body')
 ...
 * @param node
 */
export function hasPropertyDefinitionValue(node: PropertyDefinition) {
    if (!node.value)
        return false;
 ...
 * @param propertyKey
 */
export function ensurePropertyIsDefined(path: NodePath<ClassBody>, propertyKey: string) {
 ...
 * @param path
 */
export function ensureConstructorExists(path: NodePath<ClassBody>) {
    const ctor = getConstructorDefinition(path);
    if (ctor)
        return;
